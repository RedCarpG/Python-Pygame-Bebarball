...
nope_sound = load_sound("nope_01.ogg", MAIN_VOLUME * 2)
soundGroup = []
soundGroup.append(nope_sound)
soundGroup.append(laugh_sound)
# 字体
...
def main():
            ...
            # 按键事件
                ...
                if event.key == K_ESCAPE:
                    ...
                    if fullscreen:   
                        ...
                        screen.fill(BLACK)    
                        begin_text.move_center(full(begin_text.rect.center), 0)  
                        title_text.move_center(full(title_text.rect.center), 0)  
                        begin_text.blit()
                        title_text.blit()
                    # 非全屏变全屏
                    else :                             
    ...
    pause_text.move_center((width // 2, height // 2))
    restart_text.move_center((width // 2, height // 4))
    score1_text.move_center((width // 4, height // 2))
    score2_text.move_center((3 * width // 4, height // 2))
    number_text.move_center((width // 2, 2 * height // 5))
    # 主循环                
    while running:
        # 全部事件
        for event in pygame.event.get():
